...
$(function() {
    $("[name=searchID]").click(function(){
    	$('.js_userSearchName').val('');
    	$('.js_searchIdBirthInput').val('');
    	//$('#userFindgender').val('');
    	$('.js_userFindIdEmail').val('');
    	$('.js_userFindIdConfirmNumber').val('');
...
function setSearchIDStart() {
	var $container = $('.js_userFindIdcontainer');
		...
		$container.on('click','.js_confirmIdCheck',confirmIdCheck)
					...
					.on('keyPress', '.js_userFindIdConfirmNumber',passWordIdCheck)
}
...
function emailCheck_send() {
	...
	$emailConfirmNumberCheck.attr("disabled", false);
}
		...
		cbf = 
			function(bool) {
				if(bool.success) {
					...
					$emailConfirmNumberCheck.attr("disabled", true);
